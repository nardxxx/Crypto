...
function swiperMode() {
  ...
  if (mobile.matches) {
    if (!init) {
      ...
      init = true;
      document.querySelector('.merch__slider').classList.add('swiper');
      document.querySelector('.merch__row').classList.add('swiper-wrapper');
      document.querySelectorAll('.merch .merch__item').forEach(el => el.classList.add('swiper-slide'));

      const swiper = new Swiper('.merch .merch__slider', {
        // Optional parameters
        direction: 'horizontal',
        // Navigation arrows
        navigation: {
